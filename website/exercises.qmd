---
title: "Exercises — Hour 2"
subtitle: "Git for people who've been meaning to learn Git (but haven't yet)"
format:
  html:
    toc: true
    toc-depth: 2
...
These exercises take you from zero to a working Git-and-GitHub setup on your own machine. Work through them in order. Each one builds on the last, but the **collaborative finale** at the end is self-contained — even if you get stuck earlier, you can still join in.
...
2. Check you're signed in: go to **GitHub Desktop → Preferences → Accounts** (Mac) or **File → Options → Accounts** (Windows). You should see your GitHub username.
...
2. In the bottom-left, write a commit message: `Initial commit: add admissions example project`
...
```bash
git add .
git commit -m "Initial commit: add admissions example project"
```
...
4. Set visibility to **Private** — the data folder is in the project even though `.gitignore` excludes it from commits
...
## Exercise 4 · Branches *(stretch — if you have time)* {#ex4}

This exercise is for people who've finished Exercises 1–3 and want to go further. There's no pressure to complete it before the finale.
...
## Collaborative finale · Add your name {#finale}
